...
func DeployEtcdWithInstanceCount(count int, client bosh.Client, config Config, enableSSL bool, ipOffset int) (manifest etcd.Manifest, err error) {
	guid, err := NewGUID()
	if err != nil {
		return
	}
	...
	case "warden_cpi":
		...
		manifestConfig.IPOffset = ipOffset
	default:
	...
	manifest.Jobs[1], manifest.Properties = etcd.SetJobInstanceCount(manifest.Jobs[1], manifest.Networks[0], manifest.Properties, count, ipOffset)
